...
public class StaffDetailsController {
     ...
     */
    private void saveStaff(StaffMember staff, String fullName, String staffId, String staffType, Boolean isNew) {
        ...
        showAlert("Success", isNew ? "Staff details saved successfully!" : "Staff details updated successfully!", Alert.AlertType.INFORMATION);
    }
    ...
    private void deleteStaff(StaffMember staff, VBox staffCard) {
        if (Staff.deleteStaff(staff.getStaffId())) {
            ...
            showAlert("Success", "Staff deleted successfully!", Alert.AlertType.INFORMATION);
        } else {
            staffCardsContainer.getChildren().remove(staffCard);
        }
    }
    ...
    private void addNewStaff() {
        StaffMember newStaff = new StaffMember("", "", "", "", ""); // Adjust as necessary for constructor with password
